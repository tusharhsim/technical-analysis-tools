...
enctoken = '5bBEgm0GsJnWJj5nG7CrGWloapbsFGkHZYToFNVQ80mCDkkdewONR7HZ0/0Xk6Sr1DkvKoY/qJaA+/AQHYGE7TahCpXAeEDZB1kxTYobDSpi5Fb7fIF9TA=='
s_date = date(2021, 4, 15)
e_date = date(2022, 6, 20)
support_technicals = {}
resistance_technicals = {}
                                ...
                                if up_move and (low[i] > low[i+1]):
                                        if (up_move >= u_pa and down_move >= d_pa) or (up_move > u_pa*1.6):
                                                ...
                                                while sup in temp_support:
                                                    sup -= 0.1
                                                temp_support[sup] = [up_move, i-touch]
                                        down_move, up_move = 0,0
                                        floor, ceil = 0,0
                        except:
                                ...
                                if down_move and (high[i] < high[i+1]):
                                        if (up_move >= u_pa and down_move >= d_pa) or (down_move > d_pa*1.6):
                                                ...
                                                while res in temp_resistance:
                                                    res += 0.1
                                                temp_resistance[res] = [down_move, i-touch]
                                        down_move, up_move = 0,0
                                        floor, ceil = 0,0
                        except:
...
for key, value in equities.items():
        ...
        print('\nsupport')
        print(f'level\t\trejection\tcandles')
        for k,v in supports.items():
                print(f'{k}\t\t{int(v[0])}\t\t{v[1]}')
        print('\nresistance')
        print(f'level\t\trejection\tcandles')
